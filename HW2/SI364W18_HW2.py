...
@app.route('/user/<name>') #WORKS 
def hello_user(name):
    return '<h1>Hello {0}<h1>'.format(name)

@app.route('/artistform') #WORKS
def artistform():
    return render_template('artistform.html')

@app.route('/artistinfo', methods = ['GET']) #WORKS
def artistinfo():
    if request.method == "GET":
        ...
        return render_template('artist_info.html', objects=harrison['results'])
    flash('All fields are required!')
    return redirect(url_for('showform'))
   

@app.route('/artistlinks') #WORKS
def artistlinks():
    return render_template('artist_links.html')

@app.route('/specific/song/<artist_name>', methods = ['GET', 'POST']) #WORKS
